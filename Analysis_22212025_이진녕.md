...
| 2026/04/28 | 1.2.0 | 개념화 문서와의 정합성 싱크 (액터 명칭 통일 및 설계 용어 보강) | 이진녕 |
...
flowchart LR
    ...
    Owner(("👤<br>쇼핑몰 운영자/개발자<br>(Mall Owner)"))
    PG(("🏦<br>외부 PG사"))
...
*   **OCP (Open-Closed Principle):** 확장에는 열려 있어야 하지만 수정에 대해서는 닫혀 있어야 한다는 객체지향 설계 원칙(SOLID)으로, 새로운 할인 정책 추가 시 코어 로직 수정을 방지함.
*   **전략 패턴 (Strategy Pattern):** 할인 알고리즘을 캡슐화하여 런타임에 자유롭게 교체할 수 있도록 하는 디자인 패턴.
*   **DI (Dependency Injection):** 객체 간 의존 관계를 외부에서 주입하여 결합도를 낮추고 유연성을 높이는 기술 (AppConfig를 통해 구현).
...
*   **SoC (Separation of Concerns):** 프로그램의 각 부분이 독립적인 기능(관심사)을 수행하도록 모듈별로 분리하는 설계 원칙.

---

## 6. Implementation Addendum

### 6.1 Extended Use Case: 주문 취소 및 재고 복구

기존 분석 문서의 UC5는 결제 승인과 실패 복구에 초점을 맞추었으나, 설계 문서의 `OrderStatus.CANCELED` 상태와 실제 구현의 상태 전이를 일치시키기 위해 주문 취소 흐름을 확장 유스케이스로 정의한다.

| 항목 | 내용 |
| :--- | :--- |
| **Use Case** | UC6. 주문 취소 및 재고 복구 |
| **Primary Actor** | 최종 소비자 또는 운영자 |
| **Preconditions** | 주문 상태가 `COMPLETE`여야 한다. |
| **Trigger** | 주문 취소 요청이 접수된다. |
| **Main Flow** | 시스템이 주문을 조회하고, 주문 상품의 재고 row를 비관적 락으로 잠근 뒤 차감 수량을 복구한다. 이후 주문 상태를 `CANCELED`로 변경한다. |
| **Post Condition** | 주문 상태가 `CANCELED`로 저장되고, 상품 재고가 결제 이전 수량만큼 복구된다. |

### 6.2 구현 반영 범위

* 상품 관리/검색 기능은 `ProductController`와 `ProductService`로 분리하여 등록, 수정, 삭제, 검색, 상세 조회를 지원한다.
* 상품 수정은 `@Version` 기반 optimistic locking을 사용하고, 결제 재고 차감 및 주문 취소 재고 복구는 pessimistic locking을 사용한다.
* 할인 정책 설정은 `PolicyController`와 `DiscountPolicySettings`를 통해 런타임에 조회/수정 가능하다.
* `PolicyResolver`는 매 할인 계산 시점에 최신 정책 우선순위를 기준으로 정렬하여 정책 변경을 즉시 반영한다.
