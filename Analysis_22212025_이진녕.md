...
```mermaid
usecaseDiagram
    actor Customer as "최종 소비자
    (Customer)"
    actor Owner as "쇼핑몰 운영자/개발자
    (Mall Owner)"
    actor PG as "외부 PG사"

    package "OSS Mall Engine (쇼핑몰 엔진 시스템)" {
        usecase UC1 as "UC1. 상품 정보 및 실시간 재고 관리"
        usecase UC2 as "UC2. 상품 검색 및 상세 조회"
        usecase UC3 as "UC3. 할인 정책 설정 및 엔진 주입"
        usecase UC4 as "UC4. 지능형 실시간 할인 금액 산출"
        usecase UC5 as "UC5. 결제 승인 및 데이터 무결성 검증"
    }
