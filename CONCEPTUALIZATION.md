...
4.  **지능형 실시간 할인 금액 산출**
    ...
    *   **Description:** 소비자의 주문 요청 시, 엔진은 주입된 정책과 DB의 실시간 컨텍스트(회원 등급, 재고 상태 등)를 결합하여 최적의 할인 조합을 도출하고 최종가롤 산출합니다.
...
본 프로젝트가 해결하고자 하는 핵심 문제는 다음과 같습니다.

*   **정책 폭증에 따른 복잡성 증가:** 쿠폰, 등급, 타임세일, 재고 연동 등 할인 정책이 늘어날수록 규칙 간 충돌과 예외 처리가 급증합니다.
*   **폐쇄형 플랫폼의 레퍼런스 부재:** 대형 플랫폼은 알고리즘과 설계가 공개되지 않아 학습 가능한 표준 구현이 부족합니다.
*   **기존 오픈소스의 과도한 구조적 부담:** 대규모 모놀리식, 복잡한 데이터 모델(EAV 등)은 소규모/스타터 프로젝트에 높은 TCO를 유발합니다.
*   **결제/재고 일관성 확보의 난이도:** 승인 직전 검증, 동시성 처리, 재고 차감 시점 등에서 데이터 무결성이 흔히 깨집니다.

따라서 본 엔진은 **정책 계산의 확장성(OCP)**과 **결제 전후 무결성 보장**을 핵심 설계 목표로 삼습니다.
