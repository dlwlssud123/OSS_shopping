...
*   **Goals:** 결제 사고를 방지하고 주문 상태와 승인 결과의 일치성을 확보합니다. *(💡 설계 의도: 재고 차감 시점은 데이터 일관성 vs 응답 성능의 트레이드오프를 고려하여 선차감 방식을 선택함)*
...
*   **Policy Resolver (정책 해석기):** 쿠폰, 등급 등 복수의 할인 정책이 주어질 때 설정된 규칙(명시적 우선순위 필드, 단독 적용 플래그)에 따라 명확하게 할인가를 산출해 내는 시스템 모듈입니다.
...
*   **Idempotency (멱등성):** 동일 요청을 여러 번 수행해도 결과가 한 번 수행한 것과 같도록 만드는 성질로, **동일 idempotencyKey 요청 시 중복 처리 없이 기존 주문 결과를 반환**하여 결제 중복을 차단합니다.
