...
*   **지능형 정책 해결 알고리즘(Policy Resolver)의 모듈화:** 쿠폰, 등급, 타임 세일 등 복수의 할인 정책이 중첩될 때 발생하는 논리적 충돌을 해결하고 최적의 우선순위를 판별하는 엔진을 독립된 모듈로 설계합니다. 이는 기존 플랫폼들의 폐쇄적인 알고리즘을 오픈소스로 투명하게 구현하여 기술적 레퍼런스를 제시하는 과정입니다.
...
3.  **실시간 할인 연산:** 결제 요청 시 **핵심 엔진**은 주입된 정책과 **DB**의 실시간 컨텍스트(현재 재고, 회원 등급 등)를 결합함. 복수의 정책이 충돌할 경우 내부 알고리즘에 의해 최적의 할인 금액을 도출함.
4.  **결제 승인 및 검증:** 엔진은 산출된 최종 금액에 대해 **외부 PG사**와 통신하여 결제를 확정함. 승인이 완료되면 소비자에게 영수증 정보를 반환하고 DB의 재고 상태를 업데이트하며 프로세스를 종료함.
...
*   **Description:** 소비자의 주문 요청 시, 엔진은 주입된 정책과 DB의 실시간 컨텍스트(회원 등급, 재고 상태 등)를 결합하여 최적의 할인 조합을 도출하고 최종가를 산출합니다.
...
*   **Purpose:** 복수 정책 충돌 시 우선순위를 판별하여 최적 혜택을 산출합니다.
*   **Approach:** 주입된 정책과 DB의 실시간 컨텍스트(회원 등급, 재고 등)를 Policy Resolver가 결합하여 최종 할인액을 계산합니다.
*   **Dynamics:** 상품 상세 조회 시점 또는 최종 주문서 생성 단계에서 자동 수행됩니다.
*   **Goals:** 정책 간 모순을 제거하고 정밀 연산을 통해 결과 신뢰성을 확보합니다.

### 4. 결제 승인 및 무결성 검증 (Payment Validation)
*   **Purpose:** 확정 금액에 대한 금융 승인을 완결하고 데이터 무결성을 보장합니다.
*   **Approach:** 외부 PG와 통신하여 승인 처리 후, 승인 직전 재고 재확인(검증 로직)을 수행합니다.
...
* **정책 간의 논리적 충돌 방지:** 중첩된 할인 정책 적용 시 정확한 우선순위를 판별하는 'Policy Resolver'의 알고리즘 정교화가 요구됩니다.

* **실시간 재고 연동 및 무결성 확보:** 결제 승인 요청과 실제 재고 차감 사이의 시차를 극복하기 위한 정교한 트랜잭션 제어 로직이 필요합니다.

* **금융 수치 연산의 정밀도 확보:** 컴퓨터의 부동 소수점 연산 방식(IEEE 754 표준)은 금전적 계산 시 미세한 오차를 발생시킬 리스크가 있습니다. 이를 방지하기 위해 전 계층에 `BigDecimal` 등을 활용한 정밀 십진 연산 체계를 구축하여 데이터 신뢰성을 확보해야 합니다.

### 5.2 Non-Functional Requirements (NFRs)
* **Performance:** 엔진의 순수 할인액 계산 로직은 요청 후 200ms 이내에 응답을 완료해야 합니다.

* **Maintainability:** 새로운 할인 정책 추가 시 기존 결제 서비스 코드 수정률 0%를 달성해야 합니다.

* **Reliability:** 외부 시스템 오류 발생 시 주문 상태와 결제 승인 결과 간의 데이터 불일치가 발생하지 않도록 철저한 롤백 메커니즘을 갖추어야 합니다.

* **Accuracy (정확성):** 모든 할인 및 결제 금액 산출 과정에서 부동 소수점 오차율 0%를 유지해야 하며, 최종 결제 승인 금액은 원 단위 미만의 오차도 허용하지 않습니다.
...
*   **Policy Resolver (정책 해석기):** 쿠폰, 등급 등 복수의 할인 정책이 충돌할 때 우선순위를 판별하여 최적의 결과를 도출하는 본 엔진의 지능형 연산 모듈입니다.
